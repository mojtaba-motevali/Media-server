...
impl Handler<RtpCapabilitiesSettings> for Room {
    ...
    fn handle(&mut self, msg: RtpCapabilitiesSettings, ctx: &mut Context<Self>) -> Self::Result {
        ...
        let future_map = async  move {
                ...
                }.into_actor(self).map(move |producers,_act,ctx| {
                    ...
                    actix::spawn( async move {
                        ...
                        for producer in  producers {
                            ...
                            match user.create_consumer(options).await {
                                Ok(consumer) => {
                                    info!(" creating consumers while rtp_capability {:?}",consumer.id());
                                    ...
                                    ws_actor_addr.do_send(ConsumedResponse {
                                        ...
                                        app_data:*consumer.app_data().deref().clone().downcast::<MediaAppData>().unwrap().deref()
                                    });
                                    let _address = address.clone();
                                    consumer.on_producer_close( Box::new(move || {
                                        info!("producer of {:?} in rtp_setting_capa() consumer closed", consumer_id);
                                ...
                                Err(error) => {
                                    info!(" creating consumers while getting rtp_capabilities {:?}",error);
                                    address.do_send(DisconnectMessage{id,send_to_client:true,send_to_room:false,message:error.to_string()});
                                }
                            }
                            }
